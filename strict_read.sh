...
#3. Separates the grouped strings into words based on field separtation character(s). By default, the field separators are the whitespace characters, except for the newline character. More than one character can be specified by separating the characters by ','. ',' can be escaped by prefixing them with '\'.
#4. 2. and 3. occur at the same time, L->R, based on which character(s) from 2. or 3. comes first.
#5. This functionality happens in two phases. First, the input is read and some initial parsing is done separating lines and the input stored in an array by the strict_read function. Then, when the strict_get is called, the parsed array is output per-line and the result stored in the strict_line array, where each element in the array is a word.
...
display_strict_read_help_message(){
    printf "usage: Please see https://github.com/wrvenkat/bash_helper_scripts#strict_readsh"
}
